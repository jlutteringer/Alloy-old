...
public class _Reflection {
	private static final Logger logger = LogManager.getLogger(_Reflection.class);
	...
	public static <T> List<T> constructAll(List<Class<T>> unfliteredClasses, boolean filter, Object... args) {
		...
		if (filter) {
			classes = _Iterable.filter(classes, ClassType.classTypeFilter(ClassType.CONCRETE));
		}
	...
	public static <T> T invoke(Object object, String methodName, Class<T> returnType, Object... arguments) {
		return _Exception.propagate(() -> {
			return (T) getClassMethod(true, object.getClass(), methodName, _Class.getTypes(arguments))
